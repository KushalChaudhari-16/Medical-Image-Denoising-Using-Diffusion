    ...
    def __init__(self, in_channels=1, model_channels=48, channel_mult=(1, 2, 3, 4), 
        ...
        for i in reversed(range(num_resolutions)):
            ...
            for j in range(num_res_blocks + 1):
                ...
                self.ups.append(ResidualBlock(ch + skip_ch, out_ch, time_emb_dim, dropout))
                ch = out_ch
                if i in attention_resolutions and j == 0:
...
def train_diffusion_denoiser(clear_dir, noisy_dirs, img_size=512, max_samples=300, 
    ...
    for epoch in range(epochs):
        ...
        avg_train_psnr = epoch_psnr_train / (batch_count // 10) if batch_count >= 10 else 0
        ...
        print(f"Epoch {epoch+1}/{epochs} | Loss: {avg_loss:.5f} | Train PSNR: {avg_train_psnr:.2f} dB | Val PSNR: {val_psnr:.2f} dB | Val SSIM: {val_ssim:.4f}")
