...
class EnhancedNAFNet(nn.Module):
    def __init__(self, img_channel=1, width=24, middle_blk_num=8, enc_blk_nums=[1,2,2,4], dec_blk_nums=[1,1,2,2]):
        super().__init__()
        ...
        self.multiscale_conv = nn.ModuleList([
            ...
            nn.Conv2d(width, width, 5, 1, 2),
            nn.Conv2d(width, width, 7, 1, 3)
        ])
        
        self.fusion = nn.Conv2d(width * 3, width, 1, 1, 0)
    ...
    def forward(self, inp):
        ...
        x = self.fusion(torch.cat(ms_features, dim=1))
